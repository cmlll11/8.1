...
def _train_inputaware_classifier_pair(splits, config, *, pair_seed, device, output_root, smoke=False):
    ...
    if gradient_clip_norm < 0.0:
        raise ValueError("triggers.inputaware.gradient_clip_norm must be non-negative")
    opt_c = torch.optim.SGD(model.parameters(), lr=inputaware_learning_rate, momentum=float(classifier_cfg["momentum"]), weight_decay=float(classifier_cfg["weight_decay"]))
        ...
        for mask_epoch in range(mask_start, mask_epochs):
            ...
            for begin in range(0, len(order1), bs):
                ...
                if not torch.isfinite(mask_loss):
                    raise RuntimeError(f"Input-Aware mask pretraining produced non-finite loss at epoch {mask_epoch + 1}")
                mask_loss.backward()
                if gradient_clip_norm > 0.0:
                    torch.nn.utils.clip_grad_norm_(modules.mask.parameters(), gradient_clip_norm)
                opt_m.step()
    ...
    for epoch in range(start, epochs):
        ...
        for begin in range(0, len(order1), bs):
            ...
            opt_c.step(); opt_g.step(); train_loss += float(loss.detach()) * len(total_inputs); seen += len(total_inputs)
        if nonfinite_loss:
            break
